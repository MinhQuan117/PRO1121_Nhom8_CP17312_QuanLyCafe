...
import com.example.pro1121_cp17312_nhom8_quanlycafe.model.Ban;
...
public class BanAnDAO {
    ...
    public boolean ThemBanAn(String tenban){
        ...
        contentValues.put(DbHelper.TINHTRANGBAN,"false");
    ...
    public List<Ban> LayTatCaBanAn(){
        List<Ban> banAnDTOList = new ArrayList<Ban>();
        String query = "SELECT * FROM " +DbHelper.TBL_BAN;
        Cursor cursor = database.rawQuery(query,null);
        cursor.moveToFirst();
        while (!cursor.isAfterLast()){
            Ban banAnDTO = new Ban();
    ...
    public String LayTinhTrangBanTheoMa(int maban){
        ...
        cursor.moveToFirst();
        while (!cursor.isAfterLast()){
            tinhtrang = cursor.getString(cursor.getColumnIndex(DbHelper.TINHTRANGBAN));
            cursor.moveToNext();
        }
    ...
    public boolean CapNhatTinhTrangBan(int maban, String tinhtrang){
        ContentValues contentValues = new ContentValues();
        contentValues.put(DbHelper.TINHTRANGBAN,tinhtrang);
